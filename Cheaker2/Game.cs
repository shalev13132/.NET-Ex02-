...
namespace Cheaker2
{
    ...
    public class Game
    {
            ...
            while (true)
            {
                ...
                if (board.IsMoveValid(startRow, startCol, endRow, endCol, currentPlayer))
                {
                    ...
                    /*while (board.CanCapture(endRow, endCol))
                    {
                        ...
                        Console.WriteLine($"{currentPlayer.Name}'s piece at {(char)('A' + endRow)}{(char)('a' + endCol)} can perform another capture.");
                        Console.Write("Enter your next move (e.g., B3>D5): ");
                        move = Console.ReadLine();

                        if (!ParseMove(move, out startRow, out startCol, out endRow, out endCol) || startRow != endRow || startCol != endCol)
                        {
                            Console.WriteLine("Invalid move format or incorrect piece. Please try again.");
                            Console.ReadLine();
                            continue;
                        ...
                        if (!board.IsCaptureMove(startRow, startCol, endRow, endCol, currentPlayer))
                        {
                            Console.WriteLine("You must capture an opponent's piece if possible.");
                            Console.ReadLine();
                            continue;
        ...
        /* if (board.HasMandatoryCapture(currentPlayer))
         {
             Console.WriteLine("You must perform a capture move.");
         }
